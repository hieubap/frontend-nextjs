    ...
    return {
        validator: (_, number) => {
            if (number && trim(number)) {
                ...
                const phoneReplace = number.replaceAll(/\s/g,'').replaceAll('+','').replaceAll('-','').replaceAll('(','').replaceAll(')', '').replaceAll('.', '');

                if((phoneReplace.startsWith("1800") || phoneReplace.startsWith("1900")) && (phoneReplace.length !== 8 && phoneReplace.length < 10)){
                    return Promise.reject('Sai định dạng số điện thoại');
                }
                if(!phoneReplace.startsWith("1800") && !phoneReplace.startsWith("1900") && phoneReplace.length < 10) {
                    return Promise.reject('Sai định dạng số điện thoại');
                }
                if (!isRegex) {
                    return Promise.reject(message || 'Giá trị nhập vào không hợp lệ');
